...
const meilisearchCall = async (index: string, search: string, filters?: { names: Array<string>, args: string[][] }) => {
    const client = new MeiliSearch({ host: url, apiKey: api })// Use the public key not the private or master key to search.
    let filterString = [[""]]
    if (filters != undefined) {
        client.index(index).updateSettings({ filterableAttributes: filters.names })
        filterString = filters.args
    }
    return await client.index(index).search(search, { filter: filterString })
}
